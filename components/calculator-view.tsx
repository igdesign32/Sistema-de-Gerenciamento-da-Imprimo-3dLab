...
type SavedConfig = {
  ...
  materialName: string; materialCostKg: number; materialGrams: number; supplyCost: number; selectedSupplies?: SelectedSupply[]; productionHours: number;
...
export function CalculatorView({ onQuoteSaved }: { onQuoteSaved: (quote: CalculatorQuote) => void }) {
  ...
  const config = (): SavedConfig => ({ productName, quantity, colors, description, observations, extras, materialName, materialCostKg, materialGrams, supplyCost, selectedSupplies, productionHours, energyKwh, energyRate, machineRate, maintenance, riskPercent, packaging, shipping, otherCosts, feesPercent, marginPercent, unitCost: calculation.unitCost, unitPrice: calculation.unitPrice });
  const loadConfig = (id: string) => {
    setSelected(id); const item = saved.find(entry => entry.id === id); if (!item) return;
    setProductName(item.productName); setQuantity(item.quantity); setColors(item.colors); setDescription(item.description); setObservations(item.observations); setExtras(item.extras); setMaterialName(item.materialName); setMaterialCostKg(item.materialCostKg); setMaterialGrams(item.materialGrams); setSelectedSupplies(item.selectedSupplies ?? []); setProductionHours(item.productionHours); setEnergyKwh(item.energyKwh); setEnergyRate(item.energyRate); setMachineRate(item.machineRate); setMaintenance(item.maintenance); setRiskPercent(item.riskPercent); setPackaging(item.packaging); setShipping(item.shipping); setOtherCosts(item.otherCosts); setFeesPercent(item.feesPercent); setMarginPercent(item.marginPercent);
