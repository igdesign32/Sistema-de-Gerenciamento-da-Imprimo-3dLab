...
type FinanceTransaction = {
  ...
  quantity: number; unitValue: number; paymentMethod: string; status: 'Pago' | 'Em aberto'; dueDate: string; account: string; notes: string; expenseKind?: 'Fixa' | 'Variável';
};
...
export function FinanceView() {
  ...
  const income = periodTransactions.filter(item => item.type === 'Receita' && item.status === 'Pago').reduce((total, item) => total + item.amount, 0);
  const expenses = periodTransactions.filter(item => item.type === 'Despesa' && item.status === 'Pago').reduce((total, item) => total + item.amount, 0);
  ...
  return <div className="space-y-5">
    ...
    {tab === 'income' ? <div className="grid items-start gap-5 xl:grid-cols-[380px_minmax(0,1fr)]">
      ...
      <Card className="min-w-0 border-0 bg-white shadow-sm ring-1 ring-[#e6eaf0]"><CardHeader><CardTitle>Receitas lançadas</CardTitle><p className="text-xs text-slate-500">Tudo que entrou de dinheiro no período selecionado.</p></CardHeader><CardContent className="px-0">
        ...
        <div className="overflow-x-auto"><table className="w-full min-w-[920px] text-left text-xs"><thead className="bg-slate-100 text-[10px] uppercase tracking-wide text-slate-500"><tr>{['Data','Produto','Categoria','Situação','Qtd.','V. unit.','Total','Conta','Pagamento','Ações'].map(item => <th key={item} className="px-3 py-3 font-semibold">{item}</th>)}</tr></thead><tbody>{revenueRows.map(item => <tr key={item.id} className="border-t hover:bg-slate-50"><td className="whitespace-nowrap px-3 py-3 font-medium">{item.dueDate.split('-').reverse().join('/')}</td><td className="max-w-52 px-3 py-3"><p className="font-semibold">{item.product}</p>{item.notes && <p className="mt-1 truncate text-[10px] text-slate-500">{item.notes}</p>}</td><td className="px-3 py-3"><span className="rounded-full border px-2 py-1 text-[10px]">{item.category}</span></td><td className="px-3 py-3"><span className={`rounded-full px-2 py-1 text-[10px] font-semibold ${item.status === 'Pago' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>{item.status}</span></td><td className="px-3 py-3">{item.quantity}</td><td className="px-3 py-3">{brl(item.unitValue)}</td><td className={`px-3 py-3 font-bold ${item.status === 'Pago' ? 'text-emerald-600' : 'text-amber-700'}`}>{brl(item.amount)}</td><td className="px-3 py-3">{item.account}</td><td className="px-3 py-3">{item.paymentMethod}</td><td className="px-3 py-3"><div className="flex items-center gap-1"><Button variant="ghost" size="icon" aria-label={`Editar ${item.product}`} onClick={() => editTransaction(item)} className="text-[#0068ff]"><Pencil className="size-4"/></Button><Button variant="ghost" size="icon" aria-label={`Excluir ${item.product}`} onClick={() => void deleteTransaction(item)} className="text-red-500 hover:bg-red-50 hover:text-red-600"><Trash2 className="size-4"/></Button></div></td></tr>)}</tbody></table></div>
