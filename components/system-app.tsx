...
function OrdersView() {
  const [orders, setOrders] = useState<Array<{ id: string; customer: string; packageName: string; items: string; itemDetails: Array<{ name: string; quantity: number; unitPrice: number; subtotal: number }>; quantity: number; total: number; status: string; paid: number | boolean; createdAt: string }>>([]);
  ...
  const payOrder = async (orderId: string) => {
    ...
    setError('');
    try {
      const response = await fetch('/api/orders/pay', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ id: orderId }) });
      const result = await response.json() as { error?: string };
      if (!response.ok) throw new Error(result.error || 'Não foi possível registrar o pagamento.');
      setOrders(current => current.map(order => order.id === orderId ? { ...order, paid: true } : order));
    } catch (problem) {
