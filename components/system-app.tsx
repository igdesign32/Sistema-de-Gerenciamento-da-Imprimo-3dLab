  ...
  const convertQuoteToOrder = async (quote: Quote) => {
    try {
      const response = await fetch('/api/orders', {
        ...
        body: JSON.stringify({ quote: { id: quote.id, client: quote.client, item: quote.item, total: parseBrl(quote.total), totalCost: quote.cost, quantity: quote.quantity ?? 1, unitPrice: quote.unitPrice ?? parseBrl(quote.total) / Math.max(1, quote.quantity ?? 1), grams: quote.grams, hours: quote.hours, timeHours: quote.timeHours, timeMinutes: quote.timeMinutes, energyRate: quote.energyRate, machineRate: quote.machineRate, packaging: quote.packaging, fees: quote.fees, margin: quote.margin, notes: quote.notes, supplies: quote.supplies ?? [] } }),
      });
...
function OrdersView() {
  type OrderRow = { id: string; customer: string; packageName: string; items: string; itemDetails: Array<{ name: string; quantity: number; unitCost: number; unitPrice: number; subtotal: number }>; quantity: number; total: number; cost: number; profit: number; status: string; paid: number | boolean; paidAmount: number; pendingAmount: number; createdAt: string };
...
function QuoteDialog({ open, onOpenChange, onSave, onPay, onPrint, initialQuote, sequence }: { open: boolean; onOpenChange: (v: boolean) => void; onSave: (q: Quote, p: Record<string, unknown>) => Promise<boolean>; onPay: (quote: Quote, payload: Record<string, unknown>) => Promise<boolean>; onPrint: (quote: Quote) => void; initialQuote: Quote | null; sequence: number }) {
  ...
  const quoteCost = initialQuote?.cost ?? calc.cost;
  const valid = Boolean(client.trim() && item.trim() && quantity > 0 && unitPrice > 0);
  const buildQuote = (): Quote => ({
    id: initialQuote?.id ?? `ORC-${sequence}`,
    client: client.trim(), item: item.trim(), date: initialQuote?.date ?? '30 ago', total: brl(finalTotal), cost: quoteCost,
  ...
  const pay = async () => {
    ...
    await onPay(quote, { ...payload(quote), cost: quoteCost });
    setPaying(false);
  };
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-2xl">
    ...
    <div className="grid items-stretch gap-3 sm:grid-cols-3"><div className="flex min-h-24 flex-col justify-center rounded-xl bg-[#032c5e] p-4 shadow-sm"><p className="text-xs font-medium text-white/80">Custo total sincronizado</p><p className="mt-1 text-xl font-bold text-white">{brl(quoteCost)}</p></div><div className="flex min-h-24 flex-col justify-center rounded-xl bg-[#032c5e] p-4 shadow-sm"><p className="text-xs font-medium text-white/80">Lucro real</p><p className={`mt-1 text-xl font-bold ${finalTotal - quoteCost >= 0 ? 'text-emerald-300' : 'text-red-300'}`}>{brl(finalTotal - quoteCost)}</p></div><div className="flex min-h-24 flex-col justify-center rounded-xl bg-[#032c5e] p-4 shadow-sm"><label htmlFor="quote-total" className="text-xs font-medium text-white/80">Total editável</label><div className="relative mt-1"><span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-sm font-bold text-white">R$</span><Input id="quote-total" aria-label="Valor total do orçamento" type="number" min="0" step=".01" value={Number(finalTotal.toFixed(2))} onChange={event => setCustomUnitPrice(Math.max(0, Number(event.target.value) || 0) / Math.max(1, quantity))} className="h-10 border-white/35 bg-white/10 pl-10 text-lg font-bold text-white placeholder:text-white/50 focus-visible:border-white focus-visible:ring-white/30"/></div></div></div>
