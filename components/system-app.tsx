...
function Sidebar({ view, menu, onClose, onSelect, user, signOutPath }: { view: View; menu: boolean; onClose: () => void; onSelect: (v: View) => void; user: { name: string; email: string }; signOutPath: string }) {
  return <aside className={`fixed inset-y-0 left-0 z-40 flex w-[238px] flex-col bg-[var(--brand-blue)] text-white transition-transform lg:translate-x-0 ${menu ? 'translate-x-0' : '-translate-x-full'}`}>
    <div className="flex h-[76px] items-center gap-3 border-b border-white/20 px-5"><div className="grid size-10 place-items-center rounded-xl bg-[#ff6b35] shadow-[0_8px_24px_rgba(255,107,53,.28)]"><Boxes className="size-5" /></div><div><p className="text-[17px] font-bold">Forma<span className="text-[#ff8c61]">3D</span></p><p className="text-[10px] uppercase tracking-[.17em] text-white">Gestão de impressão</p></div><button className="ml-auto lg:hidden" aria-label="Fechar menu" onClick={onClose}><X className="size-5" /></button></div>
...
function SalesCartDialog({ open, onOpenChange, parts, cart, onCartChange, onFinalize, onAddToCustomer, assignedCustomer, finalizing }: { open: boolean; onOpenChange: (open: boolean) => void; parts: FinishedPart[]; cart: CartItem[]; onCartChange: (items: CartItem[]) => void; onFinalize: () => void; onAddToCustomer: () => void; assignedCustomer: Customer | null; finalizing: boolean }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[92vh] overflow-y-auto border-white/20 bg-[var(--brand-blue)] text-white sm:max-w-2xl">
...
function QuoteDialog({ open, onOpenChange, onSave, sequence }: { open: boolean; onOpenChange: (v: boolean) => void; onSave: (q: Quote, p: Record<string, unknown>) => void; sequence: number }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-2xl"><DialogHeader><DialogTitle className="text-lg">Novo orçamento</DialogTitle><DialogDescription>Informe a peça e ajuste os custos. O preço final é recalculado automaticamente.</DialogDescription></DialogHeader><div className="grid gap-4 sm:grid-cols-2"><Field label="Cliente *"><Input value={client} onChange={e => setClient(e.target.value)} placeholder="Nome do cliente"/></Field><Field label="Peça / trabalho *"><Input value={item} onChange={e => setItem(e.target.value)} placeholder="Ex.: Maquete residencial"/></Field><Field label="Material (gramas)"><Input type="number" value={grams} onChange={e => setGrams(Number(e.target.value))}/></Field><Field label="Tempo de impressão (horas)"><Input type="number" step=".5" value={hours} onChange={e => setHours(Number(e.target.value))}/></Field><Field label="Energia (R$/kWh)"><Input type="number" step=".01" value={energyRate} onChange={e => setEnergyRate(Number(e.target.value))}/></Field><Field label="Depreciação da máquina (R$/h)"><Input type="number" step=".1" value={machineRate} onChange={e => setMachineRate(Number(e.target.value))}/></Field><Field label="Embalagem (R$)"><Input type="number" value={packaging} onChange={e => setPackaging(Number(e.target.value))}/></Field><Field label="Taxas / impostos (%)"><Input type="number" value={fees} onChange={e => setFees(Number(e.target.value))}/></Field><Field label="Margem de lucro (%)"><Input type="number" value={margin} onChange={e => setMargin(Number(e.target.value))}/></Field><Field label="Observações"><Textarea placeholder="Acabamento, cor, tolerâncias..."/></Field></div><div className="grid gap-2 rounded-xl bg-[var(--brand-blue)] p-4 text-xs text-white/80 sm:grid-cols-3"><span>Material: <b className="text-white">{brl(calc.material)}</b></span><span>Energia: <b className="text-white">{brl(calc.energy)}</b></span><span>Máquina: <b className="text-white">{brl(calc.machine)}</b></span><span>Embalagem: <b className="text-white">{brl(packaging)}</b></span><span>Taxas: <b className="text-white">{brl(calc.feeValue)}</b></span><span className="text-sm text-white">Preço final: <b className="text-lg text-[#ff8358]">{brl(calc.total)}</b></span></div><DialogFooter><Button variant="outline" onClick={() => onOpenChange(false)}>Cancelar</Button><Button onClick={submit} disabled={!client.trim() || !item.trim()} className="bg-[#ff6b35] text-white hover:bg-[#e85c2b]">Salvar orçamento</Button></DialogFooter></DialogContent></Dialog>;
}

function QuickCalculator({ onCreate }: { onCreate: () => void }) { const [grams, setGrams] = useState(180); const [hours, setHours] = useState(9.5); const [margin, setMargin] = useState(35); const total = ((grams * .095) + (hours * .86) + (hours * 3.4) + 8) * 1.08 / (1 - margin / 100); return <Card className="h-fit border-0 bg-[var(--brand-blue)] py-0 text-white ring-0"><CardHeader className="border-b border-white/15 bg-black/10 py-4"><div className="flex items-center gap-3"><div className="grid size-9 place-items-center rounded-lg bg-[#ff6b35]"><Calculator className="size-4"/></div><div><CardTitle className="text-white">Orçamento rápido</CardTitle><p className="text-[11px] text-white/70">Simule o preço de uma peça</p></div></div></CardHeader><CardContent className="space-y-4 py-4"><div className="grid grid-cols-2 gap-3"><Field label="Material (g)" dark><Input type="number" value={grams} onChange={e => setGrams(Number(e.target.value))} className="border-white/20 bg-white/10 text-white"/></Field><Field label="Tempo (h)" dark><Input type="number" value={hours} onChange={e => setHours(Number(e.target.value))} className="border-white/20 bg-white/10 text-white"/></Field></div><label className="text-[11px] text-white/80">Margem <b className="float-right text-white">{margin}%</b><input aria-label="Margem" type="range" min="10" max="60" value={margin} onChange={e => setMargin(Number(e.target.value))} className="mt-2 w-full accent-[#ff6b35]"/></label><div className="rounded-xl bg-white/10 p-4"><p className="text-xs text-white/70">Preço sugerido</p><p className="text-3xl font-bold text-[#ff8358]">{brl(total)}</p></div><Button onClick={onCreate} className="w-full bg-[#ff6b35] text-white hover:bg-[#e85c2b]"><FileText/>Criar orçamento completo</Button></CardContent></Card>; }
