...
export async function POST(request: Request) {
  ...
  await env.DB.batch([
    ...
    env.DB.prepare(`INSERT INTO transactions (id, order_id, type, category, description, amount, due_at, paid_at, payment_method, created_by, created_at, updated_at) VALUES (?, NULL, 'income', 'Venda de orçamento', ?, ?, ?, unixepoch(), 'A definir', ?, unixepoch(), unixepoch()) ON CONFLICT(id) DO UPDATE SET description = excluded.description, amount = excluded.amount, due_at = excluded.due_at, paid_at = unixepoch(), updated_at = unixepoch()`)
      .bind(incomeId, incomeDescription, total, dueAt, user.userId),
    env.DB.prepare(`INSERT INTO transactions (id, order_id, type, category, description, amount, due_at, paid_at, payment_method, created_by, created_at, updated_at) VALUES (?, NULL, 'expense', 'Custo de produção', ?, ?, ?, unixepoch(), 'A definir', ?, unixepoch(), unixepoch()) ON CONFLICT(id) DO UPDATE SET description = excluded.description, amount = excluded.amount, due_at = excluded.due_at, paid_at = unixepoch(), updated_at = unixepoch()`)
      .bind(expenseId, expenseDescription, totalCost, dueAt, user.userId),
